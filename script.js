// Call main functions when page is loaded
window.onload = () => {
	getOriginals()
	getTrendingNow()
	getTopRated()
...
function fetchMovies(url, dom_element, path_type) {
	...
	// Within Fetch get response & call showMovies() with data, dom_element, & path type
}

//  ** Function that displays movies to DOM **
showMovies = (movies, dom_element, path_type) => {
	// Create variable that grabs id or class
	let moviesEl = document.querySelector(dom_element);
	
	// Loop through movies object
	for (let movie of movies.results) {
		// Within loop create img element
		let imageElement = document.createElement('img');
		// Set attribute
		imageElement.setAttribute('data-id', movie.id);
		// Set source - putting picture in element
		imageElement.src = `https://image.tmdb.org/t/p/original${movie[path_type]}`;
		
		// Append imageElement to dom_element selected
